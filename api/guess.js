...
const SYS =
  ...
  '补充标签格式为「维度：倾向」，维度含义：社交能量对应 E/I、获取信息对应 N/S、做决定对应 F/T（跟着感受走=F，讲逻辑对错=T）、生活方式对应 P/J（随性=P，计划性强=J）。' +
  '若某维度标签为「不好判断」，表示用户在该维度没有明确观察：不要把它当作任何倾向的证据，只依据行为描述推断，并相应降低该维度的把握度与整体 confidence。' +
  '判断 N/S 时注意：上进心强、重视成长、push 他人进步、常谈目标和愿景，这些是偏 N 的信号；强调固定作息、依赖经验、关注具体细节与形象管理，这些是偏 S 的信号。两类信号同时出现时，重点判断 TA 的自律与计划是服务于「未来想成为什么、想达成什么」（偏 N），还是服务于「维持既有秩序与标准」（偏 S），并在 summary 或 secondHint 中说明这层判断。' +
